...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    #   设置字体
    #---------------#
    font = ImageFont.truetype(font=str(ROOT) + '/model_data/simhei.ttf', size=20)
...
class Retinaface(object):
    ...
    #   初始化Retinaface
    #---------------------------------------------------#
    def __init__(self, encoding=0, **kwargs) -> object:
        ...
        try:
            self.known_face_encodings = np.load(ROOT / "model_data/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            self.known_face_names     = np.load(ROOT / "model_data/{backbone}_names.npy".format(backbone=self.facenet_backbone))
            self.known_face_IDs       = np.load(ROOT / "model_data/{backbone}_student_ID.npy".format(backbone=self.facenet_backbone))
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names , student_ID):
        face_encodings = []
        ...
        np.save(ROOT / "model_data/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone),face_encodings)
        np.save(ROOT /"model_data/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)
        np.save(ROOT /"model_data/{backbone}_student_ID.npy".format(backbone=self.facenet_backbone), student_ID)
    ...
    def detect_image(self, image):
        ...
        :param image: 传入的图像，色彩要求：暂时未知，RGB，BGR都可
        :return: old_image ：原本的图像
        face_names 人脸姓名,
        face_ids 人脸学号,
        text 人脸得分,
        face_number 人脸数量,
        ...
        face_names = []
        face_ids = []
        face_number = len(face_encodings)
        for face_encoding in face_encodings:
            ...
            name = "Unknown"
            id = "Unknown"
            # print("best face : face_distances" , face_distances)
            # print("best matchs : " , matches)
            #-----------------------------------------------------#
            #   取出这个最近人脸的评分
            ...
            if best_match_index != None and matches[best_match_index]:
                #print("known:" , self.known_face_names[0])
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            #   返回人脸检测坐标元组
            face_loc = (b[0] , b[1] , b[2] , b[3])
            cx = b[0]
            cy = b[1] + 12
            cv2.putText(old_image, text, (cx, cy),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255))
            cv2.putText(old_image , "Number of faces: "+ str(face_number) , (50 , 50) , cv2.FONT_HERSHEY_SIMPLEX , 1 ,(255,0,0) , 2)
        ...
        if (old_image is not None and face_names is None and face_ids is None and text is None):
            return old_image
        # if (old_image is not None and face_names is not None and face_ids is None and text is None):
        #     return old_image,face_names
        # if (old_image is not None and face_names is not None and face_ids is not None and text is None):
        #     return old_image , face
        return old_image , face_names , face_ids , text , face_number , datetime.now().strftime("%Y-%m-%d %H:%M:%S") , face_loc;
...
def registe_face(face_number , name):
    pass
